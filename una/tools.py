...
def tools_configure(install_dir: Path, arches=None):
    ...
    # For bootstrap build, ensure we don't use bld/tools/bin
    # Remove it from PATH if present
    path_parts = env.get("PATH", "").split(":")
    path_parts = [p for p in path_parts if "bld/tools/bin" not in p]
...
def tools_build(install_dir: Path):
    ...
    _get_runner().run(["ninja", f"-j{make_jobs}"], cwd=build_path, env=env, check=True)
...
def tools_install(install_dir: Path):
    ...
    _get_runner().run(["ninja", "install"], cwd=build_path, env=env, check=True)
